...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    MY_HIGHP_OR_MEDIUMP vec2 uv_scaled_centered = (floored_uv - 0.5) * 24 * max(texture_details.b, texture_details.a);
    ...
    MY_HIGHP_OR_MEDIUMP float field = (1.+ (
        ...
        cos(length(field_part3) / 363) * sin(field_part3.x / 21.92) ))/2.;
